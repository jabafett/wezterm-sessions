...
local function recreate_workspace(window, workspace_data)
	...
	-- Add nil check for foreground_process
	if foreground_process then
		...
		if
			...
			or foreground_process:find("wsl")
			or foreground_process:find("bash")
		...
		wezterm.log_info("Could not determine foreground process. Skipping exit command for initial pane.")
	end
	...
	for _, tab_data in ipairs(workspace_data.tabs) do
		...
		for j, pane_data in ipairs(tab_data.panes) do
			...
			-- TODO:This could probably be handled better in general
...
function session_manager.load_state(window)
	-- TODO: Implement a user selection screen for workspaces utilizing wezterm's lua api
...
-- TODO: Implement a possible encryption layer to protect the saved workspaces data or else wez will hunt you down
